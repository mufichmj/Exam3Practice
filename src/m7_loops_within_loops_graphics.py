...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
        ...
        corner_1 = rg.Point(corner_1.x + (2 + k * 2) * radius, corner_1.y - 2 * radius)
        corner_2 = rg.Point(corner_2.x + (4 + k * 2) * radius, corner_2.y + 2 * radius)

        hourglass(window, k + 1, hourglass_center, radius, colors[k % len(colors)])
        hourglass_center = rg.Point(hourglass_center.x + (3 + k*2)*radius, hourglass_center.y)
